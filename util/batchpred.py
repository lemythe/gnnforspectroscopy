...
if __name__ == '__main__':
    ...
    parser.add_argument('-model_type','--model_type',type=str,nargs=1,help='type of model')
    ...
    test_idx=np.loadtxt('/home/ks143235/schnet/newvalid.txt').astype(int)
 ...
 #   print(mod_list)
    ...
    for model_load in mod_list:
        ...
        if model_type == 'schnet':
           error_rse=predict(test_loader,model)
        else:
           error_rse=predict_gnn(test_loader,model)
        print(csv_name)
        csv_fin=csv_name[:-4]
        if len(csv_fin)>len(model_load): 
           res=csv_fin.replace(model_load,'')             #get diff
        else: 
           res=model_load.replace(csv_fin,'')             #get diff
        
        print(res)
        error_rse=np.array(error_rse)  
        print(np.average(error_rse))
    ...
    new_csv=csv_name[:-4]+'_upd'+'.csv'
    df.to_csv('/home/ks143235/schnet/schnetcsv/'+new_csv)    
